...
class HelmRepo < ApplicationRecord
  ...
  def client
    RepoClient.new(repo_name, helm_user.name)
  end
  ...
  def install_chart_command(version:)
    ...
    "helm install -f #{client.client_yaml_filename(version)} --create-namespace --namespace #{project_name} #{project_name} #{name}/#{project_name} --version #{version_version}"
  end
