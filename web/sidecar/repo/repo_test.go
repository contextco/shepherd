...
func TestAdd_valuesFile(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			chart, err := parent.ApplyParams(tc.params)
			if err != nil {
			...
			if err := testfixture.VerifyWithinArchive(t, ctx, store, "test-repo/test-chart-1.0.0.tgz", "test-chart/values.yaml"); err != nil {
				t.Fatalf("Failed to verify fixtures: %v", err)
			}
		})
	}
