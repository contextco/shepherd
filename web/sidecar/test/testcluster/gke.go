...
type gkeCluster struct {
	name                string
	cachedGetKubeConfig func() ([]byte, error)
}
...
func (c *gkeCluster) getKubeConfig(ctx context.Context) ([]byte, error) {
	if c.cachedGetKubeConfig == nil {
		c.cachedGetKubeConfig = sync.OnceValues(func() ([]byte, error) {
			return getGkeKubeConfig(ctx)
		})
	}

	return c.cachedGetKubeConfig()
}
...
func getGkeKubeConfig(ctx context.Context) ([]byte, error) {
	...
	name := os.Getenv("GKE_CLUSTER_NAME")

	req := &containerpb.GetClusterRequest{
		Name: fmt.Sprintf("projects/%s/locations/%s/clusters/%s", os.Getenv("GKE_PROJECT_ID"), os.Getenv("GKE_ZONE"), name),
	}
	...
	log.Printf("kubeconfig: %s", string(buf))
